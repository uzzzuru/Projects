...
# <название> = dict(<ключ>=<значение>**) - альтернативный способ создания словаря
...
# Урок 9. Множества
# Множество - это неупорядоченные последовательности элементов, каждый из которых представлен только один раз
# Тип данных - set и frozenset

# Создание множества
# <название> = {<элементы>*} - создание множества, каждый повторный элемент будет удаляться

# Альтернативный способ
# <название> = set([<элементы>*]) - альтернативный способ создания множества

# Важные заметки
# К элементам множества нельзя обратиться по индексу!
# Так же хотелось бы упомянуть, что порядок элементов в множестве определяется случайно

# Функции для работы с множествами
# add(<элемент>) - добавляет элемент в множество
# update([<элементы>*]) - добавляет несколько элементов в множество
# Также доступны и многие другие функции, которые были доступны для работы со списками, а именно это:
# pop(), clear(), remove(), copy() и remove()

# Множества "frozenset"
# frozenset - это множество, элементы в котором являются константами (неизменяемыми)


# Урок 10. Функции
# Функция - это блок кода, который может быть вызван из других частей программы
# Аргументы - это значение, передаваемое функции при её вызове в программе, носящее своё имя
# К аргументам можно обращаться только внутри тела функции
# Тип данных - function
...
# Вызов функции
# <название функции>(<аргументы функции>*) - вызов функции в программе

# Ключевые слова функций
# pass - это ключевое слово, которое используется для указания того, что блок кода функции пропускается
# return <значение> - прекращает выполнение функции, возвращая нам определённое значение
# также в return можно указать и несколько значений, тем самым функция вернёт кортеж

# Анонимная функция
# lambda <аргументы>*: <значение> - возвращает значение, полученное вызовом аргументов


# Урок 11. Работа с файлами

# Открытие файла
# open(<директория файла>, <способ открытия>, <режим открытия>) - данная функция открывает файл по определённой
# директории определённым способом открытия и в определённом режиме открытия
# после работы с файлом нужно обязательно его закрыть, чтобы не было утечки информации, для этого используется
# функция close()

# Способы открытия файла
# "r" - открытие для чтения (по умолчанию)
# "w" - открытие для записи (содержимое файла удаляется, если файла не существует, то создаётся новый)
# "x" - открытие для записи, если файла не существует, иначе исключение
# "а" - открытие для записи, но содержимое файла не удаляется (информация добавляется в конец файла)
# "+" - открытие и на запись и на чтение

# Режимы открытия файлов
# "b" - открытие файла в двоичном режиме (бинарный код)
...
# Конструкция "with"
# with open(..., encoding=<кодировка>) as <имя>:
#     <блок кода> - открывает и сохраняет файл под именем <имя> в определённой кодировке, обращаться к файлу можно
# только в блоке кода, плюс данной конструкции в том, что файл закрывается автоматически и в том


# Урок 12. Обработчик исключений
# Исключения - это механизм, который позволяет программе обрабатывать ошибки и при этом не прекращать работу
# У каждой ошибки есть своё имя

# Конструкция try / except
# try:
#     <блок кода 1>
# except <название / названия ошибок>:
#     <блок кода 2>
# Данная конструкция сначала пытается выполнить блок кода 1, и если срабатывает ошибка с определённым названием,
# или же ошибки с определёнными названиями, то срабатывает блок кода 2

# Проверка сразу всех ошибок
# except:
#     <блок кода> - позволяет обработать сразу все ошибки

# Конструкция try / except / else
# Блок else срабатывает в том случае, когда ни одно из исключений не сработало

# Конструкция try / except / else / finally
# Блок finally срабатывает после всех остальных блоков

# Получение информации об ошибке
# except ... as <имя>:
#     <блок кода> - если сработает исключение, то информация об ошибке будет доступна под именем <имя>
